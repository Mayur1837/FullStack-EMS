...
//  Send email to admin if admin does not take action on leave
  ...
  async (event, step) => {
    ...
    if (leaveApplication?.status === "PENDING") {
      ...
      await sendEmail({
        ...
        subject: `Leave Apllication Reminder`,
        body: `
            <div style="max-width: 600px;">
                <h2>Hi Admin, 👋</h2>
...
const attendanceReminderCron = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const absentEmployees = activeEmployees.filter(
      (emp) =>
        !onLeavesIds.includes(emp._id) && !checkedInIds.includes(emp._id),
    );
